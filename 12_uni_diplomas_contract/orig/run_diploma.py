...
def main():
    ...
    pub_keys = {name: common.get_public_key_from_mnemonic(
        mnemonic) for (name, mnemonic) in accounts.items()}
    priv_keys = {name: common.get_private_key_from_mnemonic(
        mnemonic) for (name, mnemonic) in accounts.items()}
    ...
    if sys.argv[1] == "deploy" or sys.argv[1] == "update":
        ...
        if sys.argv[1] == "deploy":
            ...
            app_id = create_app(
                algod_client, priv_keys[registrar], smart_contract_program, clear_program, global_schema, local_schema)

            print("Record the APP_ID {} in {}".format(app_id, CONFIG_FILE))
        elif sys.argv[1] == "update":
            # This is a update to the smart contract
            update_app(algod_client, priv_keys[registrar], APP_ID,
